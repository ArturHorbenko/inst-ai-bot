...
**Run** (verb / noun):
Executing a prompt against an Artifact or a text-only input with a chosen model, producing an output. As a noun: the stored record `(input provenance, prompt identity, model, output, timestamp, metadata)`. Artifact Runs reference exactly one Artifact and retain their prompt; text-only Runs explicitly have no Artifact and retain only a SHA-256 prompt digest plus its character length, never the raw prompt. `metadata` is optional caller-supplied namespacing such as a trait-schema and prompt-version; it does not change prompt execution. Runs are the unit the web UI displays.
